...
function TeamStructure() {
  ...
  const fetchTeamData = useCallback(async () => {
    ...
    try {
      ...
      console.log("Team structure data::", response);
      if (response?.data) {
      ...
      showToast(error.message || 'An error occurred while fetching team data', 'error');
  ...
  useEffect(() => {
    if (chartRef.current && data) {
      if (!chartInstance.current) {
        chartInstance.current = new OrgChart()
          ...
          .buttonContent(({ node, state }) => {
            const ChevronIcon = node.children ? FiChevronUp : FiChevronDown;
            return `
      ...
      onError={() => setImgSrc("https://placehold.co/40x40/d1d5db/000000?text=NA")}
    />
                  </div>
  ...
  const handleFitScreen = () => {
    if (chartInstance.current) chartInstance.current.fit();
  };

  const handleHighlightNode = () => {
    if (chartInstance.current && searchValue) {
      filterChart();
    }
  };

  const handleZoomIn = () => {
    if (chartInstance.current) chartInstance.current.zoomIn();
  };

  const handleZoomOut = () => {
    if (chartInstance.current) chartInstance.current.zoomOut();
  };
        ...
        <div className="action-toolbar mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors flex items-center gap-1"
              title="Fit Screen"
            >
              <FiMinimize size={18} />
